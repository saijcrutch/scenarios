...
def hangman_n():
    ...
    while len(word_letters) > 0:
        ...
        if user in alphabet - used_letters:
            ...
            print(computerSpeak("Your human memory must be failing you. You already used that letter."))
        elif user == '1':
            user = input("Oh so you think you know it? I'd like to see you try! ").upper()
            if user == word:
                print(computerSpeak(f"I'm speechless! I can't believe you guessed {word}. I guess you are kind of smart."))
                break
            else:
                print(computerSpeak(f"I knew you couldnt guess {word}! Guess is was too much for your memory."))
                break
        else:
        ...
        if len(word_letters) == 0:
            print(computerSpeak(f"You guessed {word}, surprisingly."))
